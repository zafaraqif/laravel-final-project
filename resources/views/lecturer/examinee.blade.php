                    ...
                    <div class="table-responsive">
                      <table class="table">
                        ...
                        <tbody>
                          @foreach($ex1 as $e)
                          <tr>
                            <td>{{Str::limit($e['proj_title'], 35, '...')}} <br/> <span style="font-size: 12px;"> {{$e['proj_type']}} </span></td>
                            <td>{{$e['student']}}</td>
                            <td>{{$e['supervisor']}}</td>
                            <td>{{($e['start_date'] == '')? 'UNINITIALIZED' : $e['start_date']." to"}} <br/> {{($e['end_date'] == '')? '' : $e['end_date']}}</td>
                            
                            @if($e['month_duration'] == '0' || $e['month_duration'] == '1')
                              <td>{{$e['month_duration']." Month"}}</td>
                            @elseif($e['month_duration'] == '')
                              <td>UNINTIALIZED</td>
                            @else
                             <td>{{$e['month_duration']." Months"}}</td>
                            @endif
